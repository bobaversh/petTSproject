...
const ErrorItem = ({ apiError }: ErrorItemProps) => {
  ...
  const getErrorMessage = () => {
    if ('status' in apiError) {
          ...
          case 400:
            if (apiError.data && typeof apiError.data === 'object') {
              ...
              if (errorData.detail) {
                ...
                if (errorData.detail.toLowerCase().includes('invalid credentials')) {
      ...
      if (apiError.data && typeof apiError.data === 'object') {
        const errorData = apiError.data as any;
        return errorData.detail || errorData.message || errorData.detailError || JSON.stringify(apiError.data);
      }
    }
